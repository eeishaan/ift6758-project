...
class MultiTaskEstimator(BaseEstimator):
    def __init__(self, gender_clf, age_clf, per_reg):
        # Independent task models
        self.age_clf = age_clf
        self.gender_clf = gender_clf
        self.per_reg = per_reg
        self.personality_keys = ['ope', 'con', 'ext', 'agr', 'neu']
    ...
    def fit(self, X, y):
        ...
        self.age_clf.fit(X, y['age'])
        print("Trained age classifier.")
        self.per_reg.fit(X, y[self.personality_keys])
        print("Trained personality regression.")
    ...
    def predict(self, X):
        ...
        personality_pred = self.per_reg.predict(X)
        for key_idx, key in enumerate(self.personality_keys):
            pred_df[key] = personality_pred[:, key_idx]
