...
from imblearn.pipeline import NewPipeline, make_pipeline
...
def test_pipeline_init_tuple():
    ...
    X = np.array([[1, 2]])
    pipe = NewPipeline((("transf", Transf()), ("clf", FitParamT())))
    pipe.fit(X, y=None)
    pipe.score(X)
...
def test_pipeline_init():
    ...
    with raises(TypeError, match=error_regex):
        NewPipeline([("clf", NoFit())])
    # Smoke test with only an estimator
    clf = NoTrans()
    pipe = NewPipeline([("svc", clf)])
    expected = dict(
...
def test_pipeline_methods_anova():
    ...
    pipe = NewPipeline([("anova", filter1), ("logistic", clf)])
    pipe.fit(X, y)
    pipe.predict(X)
...
def test_pipeline_sample_weight_unsupported():
    ...
    X = np.array([[1, 2]])
    pipe = NewPipeline([("transf", Transf()), ("clf", Mult())])
    pipe.fit(X, y=None)
    assert pipe.score(X) == 3
...
def test_pipeline_raise_set_params_error():
    ...
    pipe = NewPipeline([("cls", LinearRegression())])
...
def test_pipeline_methods_pca_svm():
    ...
    pipe = NewPipeline([("pca", pca), ("svc", clf)])
    pipe.fit(X, y)
    pipe.predict(X)
    pipe.predict_proba(X)
    pipe.predict_log_proba(X)
...
def test_pipeline_methods_preprocessing_svm():
    ...
    for preprocessing in [scaler, pca]:
        pipe = NewPipeline([("preprocess", preprocessing), ("svc", clf)])
        pipe.fit(X, y)
...
def test_fit_predict_on_pipeline_without_fit_predict():
    ...
    pipe = NewPipeline([("scaler", scaler), ("pca", pca)])
...
def test_fit_predict_with_intermediate_fit_params():
    ...
    pipe = NewPipeline([("transf", TransfFitParams()), ("clf", FitParamT())])
    pipe.fit_predict(
...
def test_pipeline_fit_transform():
    ...
    y = iris.target
    transf = Transf()
    pipeline = NewPipeline([("mock", transf)])
...
def test_set_pipeline_steps():
    transf1 = Transf()
    transf2 = Transf()
    pipeline = NewPipeline([("mock", transf1)])
...
def test_set_pipeline_step_passthrough(passthrough):
    ...
    def make():
        return NewPipeline([("m2", mult2), ("m3", mult3), ("last", mult5)])
    ...
    exp = 2 * 5
    pipeline = NewPipeline([("m2", mult2), ("m3", passthrough), ("last", mult5)])
    assert_array_equal([[exp]], pipeline.fit_transform(X, y))
    assert_array_equal([exp], pipeline.fit(X).predict(X))
    assert_array_equal(X, pipeline.inverse_transform([[exp]]))
...
def test_make_pipeline():
    ...
    assert isinstance(pipe, NewPipeline)
...
def test_pipeline_memory_transformer():
    ...
    try:
        ...
        clf = SVC(gamma="scale", probability=True, random_state=0)
        transf = DummyTransf()
        pipe = NewPipeline([("transf", clone(transf)), ("svc", clf)])
        cached_pipe = NewPipeline(
...
def test_pipeline_methods_pca_rus_svm():
    ...
    pipe = NewPipeline([("pca", pca), ("rus", rus), ("svc", clf)])
    pipe.fit(X, y)
    pipe.predict(X)
    pipe.predict_proba(X)
    pipe.predict_log_proba(X)
...
def test_pipeline_methods_rus_pca_svm():
    ...
    pca = PCA()
    rus = RandomUnderSampler(random_state=0)
    pipe = NewPipeline([("rus", rus), ("pca", pca), ("svc", clf)])
    pipe.fit(X, y)
    pipe.predict(X)
    pipe.predict_proba(X)
    pipe.predict_log_proba(X)
...
def test_pipeline_sample():
    ...
    pca = PCA()
    pipeline = NewPipeline([("pca", PCA()), ("rus", rus)])
...
def test_pipeline_sample_transform():
    ...
    pca = PCA()
    pca2 = PCA()
    pipeline = NewPipeline([("pca", pca), ("rus", rus), ("pca2", pca2)])
...
def test_pipeline_methods_anova_rus():
    ...
    rus = RandomUnderSampler(random_state=0)
    filter1 = SelectKBest(f_classif, k=2)
    pipe = NewPipeline([("rus", rus), ("anova", filter1), ("logistic", clf)])
    pipe.fit(X, y)
    pipe.predict(X)
    pipe.predict_proba(X)
    pipe.predict_log_proba(X)
...
def test_pipeline_with_step_that_implements_both_sample_and_transform():
    ...
    clf = LogisticRegression(solver="lbfgs")
    with raises(TypeError):
        NewPipeline([("step", FitTransformSample()), ("logistic", clf)])
...
def test_pipeline_with_step_that_it_is_pipeline():
    ...
    rus = RandomUnderSampler(random_state=0)
    filter1 = SelectKBest(f_classif, k=2)
    pipe1 = NewPipeline([("rus", rus), ("anova", filter1)])
    with raises(TypeError):
        NewPipeline([("pipe1", pipe1), ("logistic", clf)])
...
def test_predict_with_predict_params():
    ...
    pipe = NewPipeline([("transf", Transf()), ("clf", DummyEstimatorParams())])
    pipe.fit(None, None)
...
def test_pipeline_score_samples_pca_lof():
    ...
    pipe = NewPipeline([("rus", rus), ("pca", pca), ("lof", lof)])
    pipe.fit(X, y)
    # Check the shapes
...
parameter_grid_test_verbose = ((est, pattern, method) for
    ...
    [
     (NewPipeline([('transf', Transf()), ('clf', FitParamT())]),
      r'\[Pipeline\].*\(step 1 of 2\) Processing transf.* total=.*\n'
      r'\[Pipeline\].*\(step 2 of 2\) Processing clf.* total=.*\n$'),
     (NewPipeline([('transf', Transf()), ('noop', None),
                   ('clf', FitParamT())]),
      r'\[Pipeline\].*\(step 1 of 3\) Processing transf.* total=.*\n'
      r'\[Pipeline\].*\(step 2 of 3\) Processing noop.* total=.*\n'
      r'\[Pipeline\].*\(step 3 of 3\) Processing clf.* total=.*\n$'),
     (NewPipeline([('transf', Transf()), ('noop', 'passthrough'),
                   ('clf', FitParamT())]),
      r'\[Pipeline\].*\(step 1 of 3\) Processing transf.* total=.*\n'
      r'\[Pipeline\].*\(step 2 of 3\) Processing noop.* total=.*\n'
      r'\[Pipeline\].*\(step 3 of 3\) Processing clf.* total=.*\n$'),
     (NewPipeline([('transf', Transf()), ('clf', None)]),
      r'\[Pipeline\].*\(step 1 of 2\) Processing transf.* total=.*\n'
      r'\[Pipeline\].*\(step 2 of 2\) Processing clf.* total=.*\n$'),
     (NewPipeline([('transf', None), ('mult', Mult())]),
      r'\[Pipeline\].*\(step 1 of 2\) Processing transf.* total=.*\n'
      r'\[Pipeline\].*\(step 2 of 2\) Processing mult.* total=.*\n$'),
     (NewPipeline([('transf', 'passthrough'), ('mult', Mult())]),
      r'\[Pipeline\].*\(step 1 of 2\) Processing transf.* total=.*\n'
      r'\[Pipeline\].*\(step 2 of 2\) Processing mult.* total=.*\n$'),
